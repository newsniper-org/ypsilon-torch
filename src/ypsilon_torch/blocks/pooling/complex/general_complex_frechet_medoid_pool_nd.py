...
from ypsilon_torch.functional import frechet_medoid

from ._complex_nd_pool_base import ComplexNDPoolBase


class ComplexFrechetMedoidPoolND(ComplexNDPoolBase):
    r"""Complex N-dimensional Fréchet-medoid pooling with an arbitrary distance function.
    ...
    def _reduce(self, windows: Tensor, dim: int) -> Tensor:
        return frechet_medoid(windows, dim=dim, dist_fn=self.dist_fn)
